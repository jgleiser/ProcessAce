...
).run(tempAdminId, 'Temp Admin', tempEmail, 'hash', new Date().toISOString(), 'admin', 'active');

// 2. Generate Token
const token = jwt.sign({ id: tempAdminId, email: tempEmail, role: 'admin' }, JWT_SECRET, {
  expiresIn: '1h',
});
...
const runTest = async () => {
  try {
    console.log(
      'Testing /api/settings/verify-provider with invalid key (expecting failure or specific handling)...',
    );
    ...
    if (
      res.status === 500 &&
      res.body.error &&
      (res.body.error.includes('401') || res.body.error.includes('Incorrect API key'))
    ) {
